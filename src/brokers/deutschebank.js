...
const findDividendTax = ( content, fxRate ) => {
    ...
    if ( withholdingTaxIdx >= 0) {
        totalTax = totalTax.plus(parseGermanNum(content[withholdingTaxIdx - offset - 1].split(/\s+/)[1]))
    }
    const solidarityTaxIdx = content.indexOf('Solidaritätszuschlag');
    if ( solidarityTaxIdx >= 0) {
        totalTax = totalTax.plus(parseGermanNum(content[solidarityTaxIdx - offset].split(/\s+/)[1]))
    }
    const capitalIncomeTax = content.indexOf('Kapitalertragsteuer');
    if ( capitalIncomeTax >= 0) {
        totalTax = totalTax.plus(parseGermanNum(content[capitalIncomeTax - offset].split(/\s+/)[1]))
    }
    return +totalTax
}
...
const getDocumentType = ( content ) => {
    ...
    if ( content.includes('Dividendengutschrift') || content.includes('Ertragsgutschrift')) {
        return 'Dividend';
...
export const parsePages = pages => {
    ...
    switch (activity.type) {
        ...
        case 'Dividend': {
            const [foreignCurrency, fxRate] = findDividendForeignInformation(allPages);
            ...
            [activity.date, activity.datetime] = createActivityDateTime(findDividendDate(allPages));
