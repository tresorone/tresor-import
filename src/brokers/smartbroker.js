...
const findTax = textArr => {
  ...
  const capitalTaxIndex = textArr.findIndex(t => t.includes('Kapitalertragsteuer'));
  if (capitalTaxIndex > 0) {
    completeTax = completeTax.plus(parseGermanNum(textArr[capitalTaxIndex+2]));
  }
  const solidarityTaxIndex = textArr.findIndex(t => t.includes('Solidaritätszuschlag'));
  if (solidarityTaxIndex > 0) {
    completeTax = completeTax.plus(parseGermanNum(textArr[solidarityTaxIndex+2]));
  }
  ...
  const witholdingTaxIndex = textArr.findIndex(t => t.includes('-Quellensteuer'));
  if (witholdingTaxIndex > 0) {
    completeTax = completeTax.plus(parseGermanNum(textArr[witholdingTaxIndex+5]));
  }
  return +completeTax
}
...
export const canParsePage = (content, extension) =>
  extension === 'pdf' && content.some(line => line.includes(smartbrokerIdentificationString)) && (onvista.isBuy(content) || onvista.isSell(content) || onvista.isDividend(content));
